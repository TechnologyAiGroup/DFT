"""
Reference:https://github.com/Skyorca/OpenGDA
"""
import torch
import numpy as np
...
class CitationDomainData(InMemoryDataset):
    ...
    def __init__(self, root, name, use_pca=False, pca_dim=1000, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.dim = pca_dim
        super(CitationDomainData, self).__init__(
    ...
    def feature_compression(self, features):
        ...
        feat = sp.lil_matrix(
            PCA(n_components=self.dim, random_state=0).fit_transform(features))
        return feat.toarray()
    ...
    def process(self):
        ...
        ppmi_edge_index = torch.tensor(
            np.vstack([n_PPMI_mx.row, n_PPMI_mx.col]), dtype=torch.long)
        ...
        graph = Data(x=features, edge_index=edge_index,
                     ppmi_edge_index=ppmi_edge_index, ppmi_edge_attr=ppmi_edge_attr, y=y)
...
class BlogDomainData(InMemoryDataset):
    def __init__(self, root, name, use_pca=False, pca_dim=1000, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.use_pca = use_pca
        self.dim = pca_dim
        super(BlogDomainData, self).__init__(
            root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
...
class TwitchDomainData(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform=None, pre_filter=None):
        self.name = name
        self.root = root
        super(TwitchDomainData, self).__init__(
            root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_file_names(self):
        return [f"{self.name}_features.json", f"{self.name}_edges.csv", f"{self.name}_target.csv"]
    ...
    def _load_features(self, features_path, node_id_map):
        ...
        features = {node_id_map[int(k)]: [int(val)
                                          for val in v] for k, v in features.items()}
        return features
    ...
    def process(self):
        ...
        raw_features = self._load_features(
            self.root+"/"+f"raw/{self.name}_features.json", node_id_map)
        features = self._create_onehot_embedding(raw_features, n_nodes)
        labels = self._load_labels(
            self.root+"/"+f"raw/{self.name}_target.csv", node_id_map)
        data_list = []
        graph = Data(x=features, edge_index=edge_index,
                     ppmi_edge_index=ppmi_edge_index, ppmi_edge_attr=ppmi_edge_attr, y=labels)
        data_list.append(graph)
        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])
...
class AirPortDomainData(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.feat_dim = 8
        self.num_class = 4
        super(AirPortDomainData, self).__init__(
            root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        graph, edge_index, r_mapping, sp_adj = self.generate_core_view(
            self.root+"/"+f'raw/{self.name}-airports.edgelist', kmax=self.kmax)
        print('computing PPMI')
        ...
        labels = self.generate_label(
            self.root+"/"+f"raw/labels-{self.name}-airports.txt", r_mapping)
        data_list = []
        graph = Data(x=features, edge_index=edge_index,
                     ppmi_edge_index=ppmi_edge_index, ppmi_edge_attr=ppmi_edge_attr, y=labels)
        data_list.append(graph)
...
def dataloader_nc(model_name, dataset_name, collection_name, device="cpu"):
    ...
    dataset = dataloader(
        root=f"data/{dataset_name}/{collection_name}", name=collection_name)
    if model_name == 'pyg':
        return dataset
