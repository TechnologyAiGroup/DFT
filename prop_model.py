...
class DeProp(torch.nn.Module):
    ...
    def forward(self,  x, adj_t,edge_weight=None,type=None,test_true=False):
        
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.conv_in(x, adj_t,edge_weight,type)
        if self.with_bn:
            x = self.bn_in(x)
        if self.F_norm:
        ...
        for i, conv in enumerate(self.convs):
            x = F.dropout(x, p=self.dropout, training=self.training)
            x= conv(x, adj_t,edge_weight,type)
            if self.with_bn:
                x = self.bns[i](x)
            if self.F_norm:
                x = F.normalize(x, p=2, dim=0)
        ...
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.conv_out(x, adj_t,edge_weight,type)
